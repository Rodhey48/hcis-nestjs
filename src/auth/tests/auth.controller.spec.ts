...
describe('AuthController', () => {
    ...
    it('should cek user logged when after login', async () => {
        jest.spyOn(repository, 'findOne').mockResolvedValueOnce(data);

        const result = await controller.loggedUser({ user: data });

        expect(result).toHaveProperty('status', true);
        expect(result).toHaveProperty('message', 'Success login');
        expect(result).toHaveProperty('data');
        expect(result.data).toHaveProperty('user');
    });

    it('should failed user logged when after login', async () => {
