...
import { RequestInterface, ResponseInterface, UserLoggedInterface } from '../../@interfaces';
...
export class AuthService {
    ...
    async login(payload: LoginUserDTO) {
        const foundUser = await this.userRepo.createQueryBuilder('user')
            ...
            .orWhere("user.username = :username", { username: payload.identifier })
            .getOne()

        if (!foundUser) {
            this.logger.error(`Failed login user ${payload.identifier} due to data not found`);
            throw new BadRequestException({
        ...
        console.log(await this.bcryptService.checkPassword(payload.password, foundUser.password))

        if (!await this.bcryptService.checkPassword(payload.password, foundUser.password)) {
            this.logger.error(`Failed login user ${payload.identifier} due to password not same`);
            throw new BadRequestException({
